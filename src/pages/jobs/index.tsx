...
export function JobListPage(props: JobListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getJobs({
        relations: ['company', 'application.count', 'hiring.count', 'worklog.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<JobInterface>>(
    () => `/jobs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('application', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('hiring', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'hiring',
          header: 'Hiring',
          accessorKey: 'hiring',
          cell: ({ row: { original: record } }: any) => record?._count?.hiring,
        }
      : null,
    hasAccess('worklog', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'worklog',
          header: 'Worklog',
          accessorKey: 'worklog',
          cell: ({ row: { original: record } }: any) => record?._count?.worklog,
        }
      : null,
