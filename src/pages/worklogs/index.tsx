...
import { getWorklogs, deleteWorklogById } from 'apiSdk/worklogs';
import { WorklogInterface } from 'interfaces/worklog';

type ColumnType = ColumnDef<WorklogInterface, unknown>;

interface WorklogListPageProps {
...
export function WorklogListPage(props: WorklogListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getWorklogs({
        ...
        order: params.order,
        searchTermKeys: ['value.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<WorklogInterface>>(
    () => `/worklogs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: WorklogInterface) => {
    if (hasAccess('worklog', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/worklogs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/worklogs/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('worklog', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/worklogs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('worklog', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="worklog" />
        </Flex>

        {hasAccess('worklog', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/worklogs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
