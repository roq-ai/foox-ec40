...
import { getWorklogById } from 'apiSdk/worklogs';
import { WorklogInterface } from 'interfaces/worklog';

function WorklogViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<WorklogInterface>(
    () => (id ? `/worklogs/${id}` : null),
    () =>
      getWorklogById(id, {
        relations: ['user', 'job'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('worklog', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/worklogs/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Hours" text={data?.hours} />
