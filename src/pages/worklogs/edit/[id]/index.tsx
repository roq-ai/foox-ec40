...
import { getWorklogById, updateWorklogById } from 'apiSdk/worklogs';
...
function WorklogEditPage() {
  ...
  const handleSubmit = async (values: WorklogInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateWorklogById(id, values);
      mutate(updated);
      resetForm();
